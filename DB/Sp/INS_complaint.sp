...
EXEC DROP_db_object @tx_db_object_type = 'P', @tx_db_object_name = 'INS_complaint'
	go
...
BEGIN
	...
	BEGIN
		EXEC @g_id_return_status = GET_sp_log_level @tx_sp_name='INS_complaint', @tx_log_level=@tx_log_level OUTPUT, @id_log_level=@id_log_level OUTPUT, @is_record_time=@is_record_time OUTPUT, @is_print_msg=@is_print_msg OUTPUT, @is_persist_msg=@is_persist_msg OUTPUT
	END
	...
	SELECT @g_tx_err_msg = 'Entering[INS_complaint] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_complaint.sp', @g_id_line_num = 151
	...
	IF ( ((@id_customer_complaint_ver IS NULL) OR (@id_customer_complaint_ver = -2147483648)) )
	BEGIN
		SELECT @id_customer_complaint_ver = 0
	END

	INSERT INTO T_COMPLAINT
    (
         	id_customer_complaint_ver
        ,	-- id_customer_complaint_ver
	 is_active
	, id_env_key
	, id_user_mod_key
    ...
    (  
        @id_customer_complaint_ver
        ,--  ISNULL(@id_customer_complaint_ver			, 0)
	  ISNULL(@is_active			, 1)
	, ISNULL(@id_env_key		, 0)
	...
	, ISNULL(@id_action_key		, 0)
        ...
        , ISNULL(@dtt_txn_date 			,	'01/01/1970')			
		, ISNULL(@dtt_create 			,	GETDATE())			
		, ISNULL(@tx_atm_owner 			,	'?')			
		, ISNULL(@dec_txn_amount 		,	-2147483648)				
		, ISNULL(@tx_account_number 	,	'?')					
		, ISNULL(@tx_card_number 		,	'?')				
		, ISNULL(@tx_atm_location 		,	'?')				
		, ISNULL(@tx_comment 			,	'?')			
		, ISNULL(@tx_data_source 		,	'?')				
		, ISNULL(@id_user_mod_key		,	-2147483648)
		, ISNULL(@int_tracking_number	,	-2147483648)
    )

    SELECT @id_customer_complaint_key = IDENT_CURRENT('T_COMPLAINT')

    UPDATE T_COMPLAINT
    SET id_customer_complaint_ver = id_customer_complaint_ver + 1
    , int_tracking_number = @id_customer_complaint_key
    where id_customer_complaint_key 	  = @id_customer_complaint_key
    SELECT @g_ct_row = @@rowcount, @g_id_error_key = @@error
	SELECT @g_tx_err_msg = 'Error encountered while inserting -> T_COMPLAINT' + '@id_customer_complaint_key [' + convert(varchar, @id_customer_complaint_key) + '] '
	...
	SELECT @g_tx_err_msg = 'Exiting[INS_complaint] : @tx_action[' + ISNULL(@tx_action_name, '?') + ']'
	SELECT @g_dtt_log = GETDATE(), @g_tx_sp_name = 'INS_complaint.sp', @g_id_line_num = 418
...
GRANT EXECUTE ON INS_complaint TO app_ro, app_rw
	go