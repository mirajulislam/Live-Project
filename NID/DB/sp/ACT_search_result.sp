...
#define _SP_NAME		{ACT_search_result};
...
_CREATE_PROC (_SP_NAME)
    ...
    , @tx_force_fatch                    VARCHAR(256)            = NULL
    , @tx_data_source                    VARCHAR(256)            = NULL
    , @tx_nid_details_from               VARCHAR(256)            = NULL 
...

{
    ...
    IF ( @tx_action_name = 'NEW_SEARCH_RESULT' )
    {
        INSERT INTO _TABLE_NAME
           ...
           (  
            ...
            ,ISNULL(@tx_nid_details_from               , _DB_NULL_STR)                 
            )
    ...
    IF( @tx_action_name = _ACTION_UPDATE )
    {
        ...
        SET _TABLE_HEADER_UPD_WITH_STATE
            ...
            , tx_nid_details_from       = ISNULL(@tx_nid_details_from               , tx_nid_details_from)  
        WHERE tx_nid    = @tx_nid         
        AND   dtt_dob   = @dtt_dob
    }
    IF ( @tx_action_name = 'SELECT_REQUEST_HISTORY')
    {
            SELECT  tx_rs_type = 'RS_TYPE_NID'
             , N.*
             FROM  T_NID_SEARCH N
             WHERE L.dtt_request      >= ISNULL(CAST(@tx_from_date AS DATE)  ,L.dtt_request)
             AND CAST(L.dtt_request AS DATE) <= ISNULL(CAST(@tx_to_date AS DATE)  ,L.dtt_request)
    }
	_SP_FOOTER
}
go
