		...
		processResponse: function(request, response) {
			...
			if (!Ext.isEmpty(statusMessage)) {
				if (!Ext.isEmpty(statusMessage.payload)) {
					...
					if (isErrorStatus) {
						...
						{
							showErrorMsg('Server Error', statusMessageText);
					...
					showErrorMsg('Server Error', 'Unknown status: status message is empty');
			...
			if (!Ext.isEmpty(contentMessageArray)) {
				...
				/*else if(response.header.actionType == 'UPDATE_LOCAL_STATUS'){
					...
					showErrorMsg('Server Error', 'Content message payload must not empty');
...
function showErrorMsg(messageTitle, message){
