package com.naztech.lms.model.xml.res;

import javax.xml.bind.annotation.XmlRootElement;

@XmlRootElement(name = "RetailBaselDtls")
public class RetailBaselDtls {
	private String retBaselID;
	private String DebtHELOC;
	private String CurrFICOScore;
	private String TotalDSR;
	private String BusinessTotalScore;
	private String HasRelationship;
	private String CombinedDSR;
	private String BusinessDSR;
	private String CashAssetRatio;
	private String DebtWorthRatio;
	private String InterestTaxRatio;
	private String GeneralQuickRatio;
	private String ScoredSICCode;
	private String OwnerYears;
	private String NetWorth;
	private String DDABal;
	private String CombinedDebtRatio;
	private String ProposedLeverage;
	private String GLBLCashCoverage;
	private String CurrentRatio;
	private String LiquidityRatio;
	private String MinTangibleWorth;
	private String FICOScore;
	private String FinInqCnt;
	private String FinTradeCnt;
	private String InquiryCnt;
	private String MinorDerogatoryCnt;
	private String MajorDerogatoryCnt;
	private String NeverPastDueCnt;
	private String OpenTradeCnt;
	private String HighestCreditLmt;
	private String TradeCntThirty;
	private String TradeCntSixty;
	private String TradeCntNinety;
	private String RevolveDebtPrcnt;
	private String SatisfactoryCnt;
	private String InstLoanBal;
	private String InstLoanCnt;
	private String MortgageBal;
	private String MortgageTradeCnt;
	private String OtherTradeBal;
	private String OtherTradeCnt;
	private String RevolveBal;
	private String RevolveTradeLmt;
	private String RevolveTradeCnt;
	private String CCLimit;
	private String IsWorst;
	private String ExpenseIncomeRatio;
	private String SgmntPoolID;
	private String PoolPD;
	private String PoolLGD;
	private String PoolEAD;
	private String PD;
	private String LGD;
	private String EAD;
	private String ModelName;
	private String ModelVersion;
	private String ModelResult;
	private String IsFailed;

	// Getter Methods

	public String getRetBaselID() {
		return retBaselID;
	}

	public String getDebtHELOC() {
		return DebtHELOC;
	}

	public String getCurrFICOScore() {
		return CurrFICOScore;
	}

	public String getTotalDSR() {
		return TotalDSR;
	}

	public String getBusinessTotalScore() {
		return BusinessTotalScore;
	}

	public String getHasRelationship() {
		return HasRelationship;
	}

	public String getCombinedDSR() {
		return CombinedDSR;
	}

	public String getBusinessDSR() {
		return BusinessDSR;
	}

	public String getCashAssetRatio() {
		return CashAssetRatio;
	}

	public String getDebtWorthRatio() {
		return DebtWorthRatio;
	}

	public String getInterestTaxRatio() {
		return InterestTaxRatio;
	}

	public String getGeneralQuickRatio() {
		return GeneralQuickRatio;
	}

	public String getScoredSICCode() {
		return ScoredSICCode;
	}

	public String getOwnerYears() {
		return OwnerYears;
	}

	public String getNetWorth() {
		return NetWorth;
	}

	public String getDDABal() {
		return DDABal;
	}

	public String getCombinedDebtRatio() {
		return CombinedDebtRatio;
	}

	public String getProposedLeverage() {
		return ProposedLeverage;
	}

	public String getGLBLCashCoverage() {
		return GLBLCashCoverage;
	}

	public String getCurrentRatio() {
		return CurrentRatio;
	}

	public String getLiquidityRatio() {
		return LiquidityRatio;
	}

	public String getMinTangibleWorth() {
		return MinTangibleWorth;
	}

	public String getFICOScore() {
		return FICOScore;
	}

	public String getFinInqCnt() {
		return FinInqCnt;
	}

	public String getFinTradeCnt() {
		return FinTradeCnt;
	}

	public String getInquiryCnt() {
		return InquiryCnt;
	}

	public String getMinorDerogatoryCnt() {
		return MinorDerogatoryCnt;
	}

	public String getMajorDerogatoryCnt() {
		return MajorDerogatoryCnt;
	}

	public String getNeverPastDueCnt() {
		return NeverPastDueCnt;
	}

	public String getOpenTradeCnt() {
		return OpenTradeCnt;
	}

	public String getHighestCreditLmt() {
		return HighestCreditLmt;
	}

	public String getTradeCntThirty() {
		return TradeCntThirty;
	}

	public String getTradeCntSixty() {
		return TradeCntSixty;
	}

	public String getTradeCntNinety() {
		return TradeCntNinety;
	}

	public String getRevolveDebtPrcnt() {
		return RevolveDebtPrcnt;
	}

	public String getSatisfactoryCnt() {
		return SatisfactoryCnt;
	}

	public String getInstLoanBal() {
		return InstLoanBal;
	}

	public String getInstLoanCnt() {
		return InstLoanCnt;
	}

	public String getMortgageBal() {
		return MortgageBal;
	}

	public String getMortgageTradeCnt() {
		return MortgageTradeCnt;
	}

	public String getOtherTradeBal() {
		return OtherTradeBal;
	}

	public String getOtherTradeCnt() {
		return OtherTradeCnt;
	}

	public String getRevolveBal() {
		return RevolveBal;
	}

	public String getRevolveTradeLmt() {
		return RevolveTradeLmt;
	}

	public String getRevolveTradeCnt() {
		return RevolveTradeCnt;
	}

	public String getCCLimit() {
		return CCLimit;
	}

	public String getIsWorst() {
		return IsWorst;
	}

	public String getExpenseIncomeRatio() {
		return ExpenseIncomeRatio;
	}

	public String getSgmntPoolID() {
		return SgmntPoolID;
	}

	public String getPoolPD() {
		return PoolPD;
	}

	public String getPoolLGD() {
		return PoolLGD;
	}

	public String getPoolEAD() {
		return PoolEAD;
	}

	public String getPD() {
		return PD;
	}

	public String getLGD() {
		return LGD;
	}

	public String getEAD() {
		return EAD;
	}

	public String getModelName() {
		return ModelName;
	}

	public String getModelVersion() {
		return ModelVersion;
	}

	public String getModelResult() {
		return ModelResult;
	}

	public String getIsFailed() {
		return IsFailed;
	}

	// Setter Methods

	public void setRetBaselID(String retBaselID) {
		this.retBaselID = retBaselID;
	}

	public void setDebtHELOC(String DebtHELOC) {
		this.DebtHELOC = DebtHELOC;
	}

	public void setCurrFICOScore(String CurrFICOScore) {
		this.CurrFICOScore = CurrFICOScore;
	}

	public void setTotalDSR(String TotalDSR) {
		this.TotalDSR = TotalDSR;
	}

	public void setBusinessTotalScore(String BusinessTotalScore) {
		this.BusinessTotalScore = BusinessTotalScore;
	}

	public void setHasRelationship(String HasRelationship) {
		this.HasRelationship = HasRelationship;
	}

	public void setCombinedDSR(String CombinedDSR) {
		this.CombinedDSR = CombinedDSR;
	}

	public void setBusinessDSR(String BusinessDSR) {
		this.BusinessDSR = BusinessDSR;
	}

	public void setCashAssetRatio(String CashAssetRatio) {
		this.CashAssetRatio = CashAssetRatio;
	}

	public void setDebtWorthRatio(String DebtWorthRatio) {
		this.DebtWorthRatio = DebtWorthRatio;
	}

	public void setInterestTaxRatio(String InterestTaxRatio) {
		this.InterestTaxRatio = InterestTaxRatio;
	}

	public void setGeneralQuickRatio(String GeneralQuickRatio) {
		this.GeneralQuickRatio = GeneralQuickRatio;
	}

	public void setScoredSICCode(String ScoredSICCode) {
		this.ScoredSICCode = ScoredSICCode;
	}

	public void setOwnerYears(String OwnerYears) {
		this.OwnerYears = OwnerYears;
	}

	public void setNetWorth(String NetWorth) {
		this.NetWorth = NetWorth;
	}

	public void setDDABal(String DDABal) {
		this.DDABal = DDABal;
	}

	public void setCombinedDebtRatio(String CombinedDebtRatio) {
		this.CombinedDebtRatio = CombinedDebtRatio;
	}

	public void setProposedLeverage(String ProposedLeverage) {
		this.ProposedLeverage = ProposedLeverage;
	}

	public void setGLBLCashCoverage(String GLBLCashCoverage) {
		this.GLBLCashCoverage = GLBLCashCoverage;
	}

	public void setCurrentRatio(String CurrentRatio) {
		this.CurrentRatio = CurrentRatio;
	}

	public void setLiquidityRatio(String LiquidityRatio) {
		this.LiquidityRatio = LiquidityRatio;
	}

	public void setMinTangibleWorth(String MinTangibleWorth) {
		this.MinTangibleWorth = MinTangibleWorth;
	}

	public void setFICOScore(String FICOScore) {
		this.FICOScore = FICOScore;
	}

	public void setFinInqCnt(String FinInqCnt) {
		this.FinInqCnt = FinInqCnt;
	}

	public void setFinTradeCnt(String FinTradeCnt) {
		this.FinTradeCnt = FinTradeCnt;
	}

	public void setInquiryCnt(String InquiryCnt) {
		this.InquiryCnt = InquiryCnt;
	}

	public void setMinorDerogatoryCnt(String MinorDerogatoryCnt) {
		this.MinorDerogatoryCnt = MinorDerogatoryCnt;
	}

	public void setMajorDerogatoryCnt(String MajorDerogatoryCnt) {
		this.MajorDerogatoryCnt = MajorDerogatoryCnt;
	}

	public void setNeverPastDueCnt(String NeverPastDueCnt) {
		this.NeverPastDueCnt = NeverPastDueCnt;
	}

	public void setOpenTradeCnt(String OpenTradeCnt) {
		this.OpenTradeCnt = OpenTradeCnt;
	}

	public void setHighestCreditLmt(String HighestCreditLmt) {
		this.HighestCreditLmt = HighestCreditLmt;
	}

	public void setTradeCntThirty(String TradeCntThirty) {
		this.TradeCntThirty = TradeCntThirty;
	}

	public void setTradeCntSixty(String TradeCntSixty) {
		this.TradeCntSixty = TradeCntSixty;
	}

	public void setTradeCntNinety(String TradeCntNinety) {
		this.TradeCntNinety = TradeCntNinety;
	}

	public void setRevolveDebtPrcnt(String RevolveDebtPrcnt) {
		this.RevolveDebtPrcnt = RevolveDebtPrcnt;
	}

	public void setSatisfactoryCnt(String SatisfactoryCnt) {
		this.SatisfactoryCnt = SatisfactoryCnt;
	}

	public void setInstLoanBal(String InstLoanBal) {
		this.InstLoanBal = InstLoanBal;
	}

	public void setInstLoanCnt(String InstLoanCnt) {
		this.InstLoanCnt = InstLoanCnt;
	}

	public void setMortgageBal(String MortgageBal) {
		this.MortgageBal = MortgageBal;
	}

	public void setMortgageTradeCnt(String MortgageTradeCnt) {
		this.MortgageTradeCnt = MortgageTradeCnt;
	}

	public void setOtherTradeBal(String OtherTradeBal) {
		this.OtherTradeBal = OtherTradeBal;
	}

	public void setOtherTradeCnt(String OtherTradeCnt) {
		this.OtherTradeCnt = OtherTradeCnt;
	}

	public void setRevolveBal(String RevolveBal) {
		this.RevolveBal = RevolveBal;
	}

	public void setRevolveTradeLmt(String RevolveTradeLmt) {
		this.RevolveTradeLmt = RevolveTradeLmt;
	}

	public void setRevolveTradeCnt(String RevolveTradeCnt) {
		this.RevolveTradeCnt = RevolveTradeCnt;
	}

	public void setCCLimit(String CCLimit) {
		this.CCLimit = CCLimit;
	}

	public void setIsWorst(String IsWorst) {
		this.IsWorst = IsWorst;
	}

	public void setExpenseIncomeRatio(String ExpenseIncomeRatio) {
		this.ExpenseIncomeRatio = ExpenseIncomeRatio;
	}

	public void setSgmntPoolID(String SgmntPoolID) {
		this.SgmntPoolID = SgmntPoolID;
	}

	public void setPoolPD(String PoolPD) {
		this.PoolPD = PoolPD;
	}

	public void setPoolLGD(String PoolLGD) {
		this.PoolLGD = PoolLGD;
	}

	public void setPoolEAD(String PoolEAD) {
		this.PoolEAD = PoolEAD;
	}

	public void setPD(String PD) {
		this.PD = PD;
	}

	public void setLGD(String LGD) {
		this.LGD = LGD;
	}

	public void setEAD(String EAD) {
		this.EAD = EAD;
	}

	public void setModelName(String ModelName) {
		this.ModelName = ModelName;
	}

	public void setModelVersion(String ModelVersion) {
		this.ModelVersion = ModelVersion;
	}

	public void setModelResult(String ModelResult) {
		this.ModelResult = ModelResult;
	}

	public void setIsFailed(String IsFailed) {
		this.IsFailed = IsFailed;
	}
}